...
- **Deviation from independence** is measured by statistic **D**:  
  ...
  D = q_{12} - q_1 q_2
  \]
- If **D ≠ 0**, the SNPs are correlated.
...
- **r² (squared correlation coefficient)**  
  ...
  - \( r² = 1 \) → SNPs provide identical information.  
  - \( r² = 0 \) → SNPs are completely unlinked.  
  - In practice, GWAS often consider SNPs with \( r² > 0.8 \) as strongly correlated.  
...
3. **Filtering Redundancy**
   - If multiple SNPs are perfectly correlated (r² ≈ 1), only one needs to be tested.  
